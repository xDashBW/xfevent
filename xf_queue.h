...
static inline void __xf_queue_push(xf_queue_t * node, xf_queue_t *prev, xf_queue_t *next) {
    node->prev = prev;
    node->next = next;
    prev->next = node;
    next->prev = node;
...
static inline void xf_queue_push_back(xf_queue_t *head, xf_queue_t * node) {
    __xf_queue_push(node, head->prev, head);
}

static inline void xf_queue_push_head(xf_queue_t *head, xf_queue_t *node) {
    __xf_queue_push(node, head, head->next);
}
    
static inline void xf_queue_del(xf_queue_t *node) {
